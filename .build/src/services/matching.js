...
class MatchingService {
     ...
     */
    static validarVenta(voucher, notificacion) {
        ...
        const camposCoincidentes = Object.keys(checks).filter((k) => checks[k]);
     ...
     */
    static formatearMensajeExito(voucher, notificacion) {
...
• Fecha: ${notificacion.fecha_hora ? new Date(notificacion.fecha_hora).toLocaleString('es-PE') : 'N/A'}`;
     ...
     */
    static formatearCamposNoCoincidentes(checks, notificacion, voucher) {
        const noCoincidentes = [];
        if (!checks.monto) {
            noCoincidentes.push(`• Monto: Notificación S/${notificacion.monto?.toFixed(2)} ≠ Voucher S/${voucher.monto.toFixed(2)}`);
        }
        if (!checks.codigoSeguridad) {
            noCoincidentes.push(`• Código Seguridad: Notificación ${notificacion.codigo_seguridad} ≠ Voucher ${voucher.codigoSeguridad}`);
        }
     ...
     */
    static generarMensajeDuplicado(numeroOperacion, vendedor, fecha) {
